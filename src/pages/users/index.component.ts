...
export class BaseUsersComponent extends BaseDataComponent {
  ...
  deActive(item: any) {
    this.dialogService.prompt({title: 'پیام برای کاربر'}, {title: 'غیرفعال کردن', position: 'top-center', maskClosable: true}).then(e => {
      if (!e.result) return;
  ...
  showMessage(item: any) {
    this.dialogService.prompt({title: 'پیام برای کاربر', value: item.message}, {title: 'تنظیم پیام کاربر', position: 'top-center', maskClosable: true}).then(e => {
      if (e.result) {
